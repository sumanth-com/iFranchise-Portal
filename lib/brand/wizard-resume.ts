...
function isStep1Complete(brand: Brand, _assets: BrandAssetsBundle): boolean {
...
function isStep2Complete(_brand: Brand, assets: BrandAssetsBundle): boolean {
  return Boolean(assets.logo);
}

function isStep3Complete(brand: Brand, _assets: BrandAssetsBundle): boolean {
  return brand.investment_min != null && brand.franchise_fee != null;
}

function isStep4Complete(brand: Brand, _assets: BrandAssetsBundle): boolean {
  return (brand.franchise_models?.length ?? 0) > 0;
}

function isStep5Complete(brand: Brand, _assets: BrandAssetsBundle): boolean {
  return (
...
function isStep6Complete(brand: Brand, _assets: BrandAssetsBundle): boolean {
  return (
...
function isStep7Complete(brand: Brand, assets: BrandAssetsBundle): boolean {
  return isStep2Complete(brand, assets) && isStep1Complete(brand, assets);
}

function isStep8Complete(brand: Brand, _assets: BrandAssetsBundle): boolean {
